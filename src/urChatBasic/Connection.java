...
public class Connection implements Runnable{
	...
	private void serverMessage(String receivedText) throws IOException{
		String[] receivedOptions = receivedText.substring(nthOccurrence(receivedText, ':', 1)+1, nthOccurrence(receivedText, ':', 2)).split(" ");
		...
		message = receivedText.substring(nthOccurrence(receivedText, ':', 2)+1);
		 ...
		 if (receivedOptions[0].toLowerCase( ).startsWith("ping")) {
        	 ...
        	 if(receivedOptions.length > 1)
	        	 switch(receivedOptions[1]){
		        	...
		        	case "332" :String incomingTopic = "";
			    				for(int x = 4; x < receivedOptions.length; x++)
			    					incomingTopic += " "+receivedOptions[x];
			    				if(incomingTopic.length() > 3)
			    					gui.setChannelTopic(receivedOptions[3], incomingTopic.substring(1));
		        				break;
		        	//353 = User List
		        	...
		        	case "JOIN":if(extractNick(receivedOptions[0]).equals(myNick)){
		        				...
		        				break;
		        	case "PRIVMSG": if(!receivedOptions[2].contains(myNick)){
		        						gui.printChannelText(receivedOptions[2],"<"+extractNick(receivedOptions[0])+"> "+message.substring(2),extractNick(receivedOptions[0]));
		        					...
		        					if(message.contains(myNick))
			        					...
			        					gui.printChannelText(myNick,"{"+extractNick(receivedOptions[0])+"} "+message.substring(2),extractNick(receivedOptions[0]));
